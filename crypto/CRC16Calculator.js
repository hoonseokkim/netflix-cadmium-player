/**
 * Netflix Cadmium Player — CRC-16/CCITT Calculator
 *
 * Computes CRC-16 checksums using the CCITT polynomial (0x8005).
 * Used for data integrity verification in media segment parsing
 * and manifest validation.
 *
 * @module crypto/CRC16Calculator
 * @original Module_25154
 */

/**
 * Pre-computed CRC-16 lookup table (256 entries) using the CCITT polynomial.
 * Each entry is the CRC value for a single byte value at that index.
 * @type {Uint16Array}
 */
...
/**
 * Calculate a CRC-16/CCITT checksum over an array of bytes.
 *
 * @param {Uint8Array|number[]} data - Input byte array.
 * @param {number} [initialValue=0] - Initial CRC value (for chaining).
 * @returns {number} 16-bit CRC value.
 */
export function calculateCRC(data, initialValue = 0) {
  let crc = ~~initialValue;
  for (let i = 0; i < data.length; i++) {
    crc = (CRC16_TABLE[(crc >> 8 ^ data[i]) & 255] ^ (crc << 8)) & 65535;
  }
  return crc;
}
